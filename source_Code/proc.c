...
{
  int digunakan_s;
  int digunakan_ms = ticks - (p -> start_ticks);
  digunakan_s = digunakan_ms/1000;
  digunakan_ms = digunakan_ms % 1000;

  char* mulai= "";
  if(digunakan_ms < 100 && digunakan_ms > 10){
    mulai = "0";
  }
  if(digunakan_ms < 10){
    mulai = "00";
  ...
  cprintf("%d\t%s\t%s%d.%s%d\t%s\t%d\t",
	p->pid, p->name, "     ", 
	digunakan_s, mulai, digunakan_ms,
	states[p->state], p->sz);
  return;
}
#endif
